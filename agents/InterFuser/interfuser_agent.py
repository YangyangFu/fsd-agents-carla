...
class InterfuserAgent(AutonomousAgent):
    def setup(self, config_path):
        ...
        self.skip_frames = 1
        self.controller = InterfuserController(self.config.controller)
        ...
        if self.ensemble:
            for i in range(len(self.config.model)):
                self.nets = []
                net = AGENTS.build(self.config.model[i])
                path_to_model_file = self.config.checkpoints[i]
            ...
            self.net = AGENTS.build(self.config.model)
            path_to_model_file = self.config.checkpoints
            print('load model: %s' % path_to_model_file)
        ...
        self.momentum = 0. #self.config.momentum
    ...
    def run_step(self, input_data, timestamp):
        ...
        input_data = {}
        input_data['img'] = [rgb, rgb_left, rgb_right, rgb_center]
        input_data['pts'] = torch.from_numpy(tick_data["lidar"]).float().cuda().unsqueeze(0)
        input_data['goal_points'] = torch.from_numpy(tick_data["target_point"]).float().cuda().view(1, -1)
        input_data['ego_velocity'] = torch.tensor([velocity]).float().cuda().view(1, -1)

        if self.ensemble:
            outputs = []
            with torch.no_grad():
                for net in self.nets:
                    output = net(input_data, None, mode='predict')
            ...
            with torch.no_grad():
                output = self.net(input_data, None, mode='predict')
            ## deal with output
            traffic_meta = output['object_density']
            is_junction = output['junction']
            stop_sign = output['stop_sign']
            traffic_light_state = output['traffic_light']
